    ...
    <>
        ...
        <div className="container-site py-8 lg:py-10">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 bg-card border border-border rounded-lg px-5 py-3">
              ...
              <div>
                <p className="text-2xl font-bold text-primary">{String(properties.length).padStart(2, "0")}</p>
                <p className="text-[0.65rem] text-muted-foreground">Properties Saved</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="sticky top-24 space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-foreground">Book a Consultation</p>
                          <p className="text-[0.65rem] text-muted-foreground">Get expert guidance for your saved properties.</p>
                        </div>
                        <ArrowRight className="w-3.5 h-3.5 text-muted-foreground group-hover:text-accent shrink-0" />
                      </Link>
                      <Link
                        href="/contact"
                        className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted transition-colors group"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-foreground">Talk to an Expert</p>
                          <p className="text-[0.65rem] text-muted-foreground">Speak with our relationship manager.</p>
                        </div>
                        <ArrowRight className="w-3.5 h-3.5 text-muted-foreground group-hover:text-accent shrink-0" />
                      </Link>
                      <button
                        onClick={() => { setShowShare(true); generateShareLink(); }}
                        className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted transition-colors group w-full text-left cursor-pointer"
                      >
                        <div className="w-9 h-9 rounded-full bg-accent/10 flex items-center justify-center shrink-0">
                          <Share2 className="w-4 h-4 text-accent" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-foreground">Share Your List</p>
                          <p className="text-[0.65rem] text-muted-foreground">Share your saved properties with family or advisors.</p>
                        </div>
                        <ArrowRight className="w-3.5 h-3.5 text-muted-foreground group-hover:text-accent shrink-0" />
                      </button>
                    </div>
                  ...
                  <div className="bg-card border border-border rounded-lg p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-foreground mb-1">Secure & Private</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {properties.map((property) => (
                    <Link
                      ...
                      className="bg-card border border-border rounded-lg overflow-hidden group block hover:shadow-md transition-shadow"
                    >
                      {/* Image */}
                      <div className="relative aspect-[16/10] overflow-hidden">
                        ...
                        <div className="absolute top-3 right-3 z-10">
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-card/90 border border-border flex items-center justify-center text-destructive hover:text-destructive hover:border-destructive-border hover:bg-card transition-colors cursor-pointer disabled:opacity-50"
                            aria-label="Remove from saved"
                          >
                            {removing === property.id ? (
                              <div className="w-3.5 h-3.5 border-2 border-destructive-border border-t-red-500 rounded-full animate-spin" />
                            ) : (
      ...
      {showShare && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-scrim" onClick={() => setShowShare(false)} />
          <div className="relative bg-card rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div className="flex items-center gap-2">
                <Share2 className="w-4 h-4 text-accent" />
                <h3 className="text-sm font-bold text-foreground">Share Your Property List</h3>
              </div>
              <button onClick={() => setShowShare(false)} className="p-1 hover:bg-secondary rounded cursor-pointer">
